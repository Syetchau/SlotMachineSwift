...
//  SlotMachineSwift
//
//  Created by LiewSyetChau on 28/12/21.
//
...
struct ContentView: View {
    ...
    @State private var numbers = Array(
        repeating: 0, count: 9)
    @State private var backgrounds = Array(
        repeating: Color.white, count: 9)
    ...
    var body: some View {
        ...
        ZStack {
            ...
            //rotation of lighter background
            Rectangle().foregroundColor(
                Color(
                    red: 228/255,
                    green: 195/255,
    ...
    func processResults(_ isMax: Bool = false) {
        ...
        self.backgrounds =
            self.backgrounds.map { _ in
                Color.white
            }
        
        if isMax {
            //spin all card, change all images
            self.numbers = self.numbers.map({ _ in
                Int.random(in: 0...self.symbols.count - 1)
            })
        } else {
            //spin middle row only
